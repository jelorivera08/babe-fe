...
const CreateOrderContainer = ({ open, handleClose, user }) => {
  ...
  const handleConfirmClick = (values) => {
    const variables = {
      ...
      productInput: Object.values(values.products).filter(
        (v) => parseInt(v.quantity, 10) > 0,
      ).map(
        (v) => ({ ...v, quantity: parseInt(v.quantity, 10) }),
      ),
    };
    ...
    commitMutation(
      AppEnvironment,
      {
        ...
        updater: (store) => {
          ...
          regionalStockistProxy.setLinkedRecords([...payload], 'orders');
        },
        onCompleted: () => {
          setOrderProducts({});
          handleClose(false);
            ...
            <Table celled>
              ...
              <Table.Body>
                {products.map((product) => {
                  if (orderProducts[product.name]) {
                    totalAmount += orderProducts[product.name].amount
                    ...
                    <Table.Row key={product.name}>
                      ...
                      <Table.Cell>
                        <input
                          ...
                          value={
                          orderProducts[product.name] ? orderProducts[product.name].quantity : 0
                        }
                          onChange={(e) => {
                            setOrderProducts({
                              ...orderProducts,
                      ...
                      <Table.Cell>
                        {orderProducts[product.name]
                          ? `₱ ${formatNumber((orderProducts[product.name].quantity * orderProducts[product.name].amount).toFixed(2))}`
                          : 0}
                      </Table.Cell>
                    </Table.Row>
                  );
              ...
              <Table.Footer>
                <Table.Row>
                  ...
                  <Table.HeaderCell>{`₱ ${formatNumber((totalAmount).toFixed(2))}`}</Table.HeaderCell>
                </Table.Row>
              </Table.Footer>
            </Table>
          </div>
        ...
        <Button
          ...
          disabled={
            ...
            || Object.values(orderProducts).filter(
              (v) => Number.parseInt(v.quantity, 10) <= 0
          ...
          onClick={() => handleConfirmClick({ products: orderProducts, orderDate, user })}
        />
      </Modal.Actions>
    </Modal>
  );
